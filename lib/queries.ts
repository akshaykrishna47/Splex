...
export function useTrips() {
  return useQuery({ queryKey: keys.trips, queryFn: () => repo.trips.list() });
}
...
export function useCreateTrip() {
  ...
  return useMutation({
    ...
    onSuccess: () => void client.invalidateQueries({ queryKey: keys.trips }),
  });
}
...
/** Anything that moves money invalidates the whole trip's derived state. */
function invalidateTrip(client: QueryClient, tripId: Uuid): void {
  void client.invalidateQueries({ queryKey: keys.expenses(tripId) });
  void client.invalidateQueries({ queryKey: keys.balances(tripId) });
